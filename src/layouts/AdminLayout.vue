  ...
  <div class="shell">
    <aside class="sidebar" :class="{ open: menuAbierto }">
      ...
      <div class="sidebar-foot">
        ...
        <button class="btn btn-ghost btn-sm" style="width:100%; margin-top:.6rem;" @click="cerrarSesion">Cerrar sesión</button>
      </div>
    </aside>
...
.sidebar {
  width: 260px; flex-shrink: 0; background: var(--color-forest-deep); color: #EDE7D4;
  display: flex; flex-direction: column; padding: 1.3rem 1rem; position: sticky; top: 0; height: 100vh; overflow-y: auto;
}
.brand { display: flex; align-items: center; gap: .7rem; padding: .3rem .3rem 1.2rem; border-bottom: 1px solid rgba(237,231,212,0.12); margin-bottom: 1rem; }
.brand-mark {
  width: 38px; height: 38px; border-radius: var(--radius-sm); background: var(--color-gold); color: #241800;
  display: flex; align-items: center; justify-content: center; font-family: var(--font-display); font-weight: 700; font-size: 1rem;
}
.brand-title { font-family: var(--font-display); font-size: 1rem; margin: 0; }
.brand-sub { margin: 0; font-size: .72rem; color: #B9AF8F; }

.nav { flex: 1; display: flex; flex-direction: column; gap: .15rem; }
.nav-group { font-size: .68rem; text-transform: uppercase; letter-spacing: .07em; color: #9C9377; margin: .9rem .5rem .35rem; }
.nav-link {
  display: flex; align-items: center; gap: .65rem; padding: .55rem .7rem; border-radius: var(--radius-sm);
  text-decoration: none; color: #E4DEC7; font-size: .875rem; font-weight: 500; transition: background .12s ease;
}
.nav-link:hover { background: rgba(237,231,212,0.08); }
.nav-link.active { background: var(--color-gold); color: #241800; font-weight: 700; }
.nav-link svg { flex-shrink: 0; }

.sidebar-foot { border-top: 1px solid rgba(237,231,212,0.12); padding-top: 1rem; margin-top: .8rem; }
.user-chip { display: flex; align-items: center; gap: .6rem; text-decoration: none; color: inherit; padding: .3rem; border-radius: var(--radius-sm); }
.user-chip:hover { background: rgba(237,231,212,0.08); }
.avatar { width: 34px; height: 34px; border-radius: 50%; background: #3A5240; display: flex; align-items: center; justify-content: center; font-size: .78rem; font-weight: 700; }
.user-info { display: flex; flex-direction: column; line-height: 1.25; }
.user-info strong { font-size: .84rem; }
.user-info small { color: #B9AF8F; font-size: .72rem; }

.main { flex: 1; min-width: 0; display: flex; flex-direction: column; }
.topbar { display: flex; align-items: center; gap: .8rem; padding: 1rem 1.6rem; border-bottom: 1px solid var(--color-border); background: var(--color-surface); position: sticky; top: 0; z-index: 20; }
.page-title { font-size: 1.15rem; }
.topbar-spacer { flex: 1; }
.hamburger { display: none; background: none; border: none; font-size: 1.2rem; cursor: pointer; color: var(--color-forest); }
...
@media (max-width: 920px) {
  .sidebar { position: fixed; z-index: 40; transform: translateX(-100%); transition: transform .2s ease; box-shadow: var(--shadow-pop); }
  .sidebar.open { transform: translateX(0); }
  .hamburger { display: block; }
  .backdrop { display: block; position: fixed; inset: 0; background: rgba(22,38,27,0.4); z-index: 30; }
